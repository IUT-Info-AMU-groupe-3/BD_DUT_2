...
---5)---
  WITH NOUVELLE_VENTES AS(
    SELECT IDV,IDM,IDP,V.IDT,QUANTITE,PRIX
    FROM VENTE V JOIN TEMPS TPS ON V.IDT = TPS.IDT
    WHERE DATEV >= TO_DATE('01/'||TO_CHAR(SYSDATE,'MM/YYYY'),'DD/MM/YYYY')
  )
  ---INSERT INTO CVENTE
  SELECT IDM,IDP,IDT,Sum(QUANTITE), SUM(PRIX) FROM NOUVELLE_VENTES GROUP BY IDP,IDM,IDT 
  UNION
  SELECT IDM,'ALL',IDT,Sum(QUANTITE), SUM(PRIX) FROM NOUVELLE_VENTES GROUP BY IDM,IDT 
  UNION
  SELECT 'ALL',IDP,IDT,Sum(QUANTITE), SUM(PRIX) FROM NOUVELLE_VENTES GROUP BY IDP,IDT 
  UNION
  SELECT 'ALL','ALL',IDT,SUM(QUANTITE), SUM(PRIX) FROM NOUVELLE_VENTES GROUP BY IDT;
...
(
  ...
  IDL5 VARCHAR2(10), 
  NOML VARCHAR2(10),
  CONSTRAINT PK_DLIEU PRIMARY KEY(IDD1)
)
