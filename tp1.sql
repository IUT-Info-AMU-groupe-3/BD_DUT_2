...
<--5)-->
SELECT CI.CONSTRAINT_NAME, COl.CO
LUMN_NAME,COL.DATA_TYPE FROM USER_CONSTRAINTS CI, USER_TAB_COLUMNS COL WHERE CI.TABLE_NAME = COL.TABLE_NAME AND CI.COLUMN_NAME = COL.TABLE_NAME AND CI.TABLE_NAME IN (SELECT TABLE_NAME FROM USER_TAB_COLUMNS C WHERE C.DATA_TYPE = 'NUMBER');
