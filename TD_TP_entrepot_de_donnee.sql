...
DECLARE 
  lastupd DATE;
BEGIN
  SELECT MAX(DATEV) INTO lastupd FROM TEMPS JOIN CVENTE ON TEMPS.IDT = CVENTE.IDT;
  DBMS_OUTPUT.PUT_LINE(lastupd);
  UPDATE_VENTE (lastupd);
END;
---6)--- 
DROP TABLE D_LIEU;
CREATE TABLE D_LIEU (
...
SELECT IDL, NOM FROM LIEU CONNECT BY PRIOR IDLPERE = IDL START WITH IDL = 8505;

CREATE OR REPLACE PROCEDURE FILL_D_LIEU AS
  REQ VARCHAR2(100);
  VALS VARCHAR2(100);
  COLS NUMBER;
  CURSOR LIEUX IS SELECT IDL, NOM FROM LIEU;
  CURSOR HIERARCHIE (START_IDL VARCHAR2) IS SELECT IDL FROM LIEU CONNECT BY PRIOR IDLPERE = IDL START WITH IDL = START_IDL;
BEGIN 
  FOR L IN LIEUX LOOP
    REQ := 'INSERT INTO D_LIEU VALUES (''' || L.IDL || ''',';
    VALS := '';
    FOR I IN HIERARCHIE (L.IDL) LOOP 
      VALS := '''' || I.IDL || ''',' || VALS;
      COLS := HIERARCHIE%ROWCOUNT;
    END LOOP;
    FOR I IN 1 .. 5-COLS LOOP 
      VALS := VALS || '''NULL'',';
    END LOOP;
    VALS := VALS || '''' || L.NOM || ''')';
    DBMS_OUTPUT.PUT_LINE(REQ || VALS);
    REQ := REQ || VALS;
